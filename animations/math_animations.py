...
def animate_bouncing_ball():
    ...
    for frame in range(60):
        clear()
        grid = [[" " for _ in range(width)] for _ in range(height)]
        for i in range(width): grid[0][i] = grid[height-1][i] = "─"
        for i in range(height): grid[i][0] = grid[i][width-1] = "│"
        grid[0][0], grid[0][-1], grid[-1][0], grid[-1][-1] = "┌","┐","└","┘"
        x += dx; y += dy
        if x <= 1 or x >= width-2: dx *= -1
        if y <= 1 or y >= height-2: dy *= -1
        grid[y][x] = "●"
        print("\n".join("".join(row) for row in grid))
        time.sleep(0.1)
...
def animate_wave():
    ...
    for frame in range(60):
        ...
        for x in range(width):
            ...
            for y in [y1,y2,y3]:
                if 0 <= y < height: grid[y][x]=random.choice(["~","≈","∼"])
        print("\n".join("".join(row) for row in grid))
        time.sleep(0.1)
...
def animate_dna_helix():
    ...
    for frame in range(50):
        ...
        for y in range(height):
            ...
            if 0<=x1<width: grid[y][x1]="O"
            if 0<=x2<width: grid[y][x2]="O"
...
def animate_spiral_galaxy():
    ...
    for frame in range(80):
        ...
        for _ in range(20):
            ...
            if grid[y][x]==" ": grid[y][x]=random.choice(["·","‧","∘"])
        print("\n".join("".join(row) for row in grid))
        time.sleep(0.12)
